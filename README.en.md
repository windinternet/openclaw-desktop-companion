...
This plugin is not a single-purpose artifact plugin. It is the runtime
collaboration layer between OpenClaw Desktop and OpenClaw Gateway: the plugin
runs on the Gateway side and owns context injection, Agent tool registration,
session artifact observation, and `node.invoke` forwarding; OpenClaw Desktop
connects to the same Gateway as a Gateway node and executes structured local
commands for files, Git, previews, and outputs.
